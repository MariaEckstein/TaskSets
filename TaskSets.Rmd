...
for (filename in filenames) {
  ...
  demos = gsub('[[:punct:]]', '', as.character(subj_file$responses)[1])
  subjID = as.numeric(strsplit(strsplit(demos, "Q0")[[1]][2], "Q1")[[1]][1])
  subj_file$subjID = subjID
  
  # Get demographics
  demographics[demo_row, "subjID"] = subjID
  dob = gsub("[a-zA-Z]", "", strsplit(strsplit(demos, "Q1")[[1]][2], "Q2")[[1]][1])
  month = substr(dob, 1, 2)
  day = substr(dob, 3, 4)
  year = substr(dob, 5, nchar(dob))
  ...
  today = gsub("[a-zA-Z]", "", strsplit(demos, "Q2")[[1]][2])
  month = substr(today, 1, 2)
  if (month == "19") {
    month = "10"
  }
  day = substr(today, 3, 4)
  year = substr(today, 5, nchar(today))
  if (nchar(year) == 1) {
    day = paste("0", substr(today, 3, 3), sep = "")
    year = substr(today, 4, nchar(today))
  }
  if (nchar(year) == 2) {
...
for (phas in feed_aliens_phases) {
  ...
  if (run_regression) {
    ...
    intrusions_form = "ACC ~ TS_values + av_reward_ACC_TS + alien_trial_sum + (TS_values + av_reward_ACC_TS + alien_trial_sum | subjID)"
...
for (phas in feed_aliens_phases) {
  ...
  if (gg_save) {
    ...
    ggsave(paste(plot_dir, "/2learning_curves_TS_", phas, ".png", sep = ""), learning_curves_TS, width = 3, height = 3)
...
if (run_regression) {
  for (phas in feed_aliens_phases) {
    ...
    trial_value_form = "ACC ~ TS_value + av_reward + alien_trial_sum + (TS_value + av_reward + alien_trial_sum | subjID)"
