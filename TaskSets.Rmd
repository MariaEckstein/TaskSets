...
for (phas in feed_aliens_phases) {
  ...
  sum_dat = ddply(dat, .(subjID, evidence), summarize,
                  ACC = mean(ACC, na.rm = T))
  ggplot(sum_dat, aes(ACC, fill = TS)) +
  ...
  gg_value_guess_paper = ggplot(sum_dat, aes(evidence, 100 * ACC)) +  # , color = cum_ACC_1back, group = cum_ACC_1back
...
if (run_regression) {
  for (phas in feed_aliens_phases) {
    ...
    trial_value_form = "ACC ~ alien_trial_sum + av_reward * TS_value + (1 | subjID)"
