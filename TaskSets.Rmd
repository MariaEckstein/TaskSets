...
n_aliens = 4
data_dir = "C:/Users/maria/MEGAsync/Berkeley/TaskSets/Data/17-11-13"
...
learning_curves + aes(alien_trial, rt, color = phase) + coord_cartesian(y = c(400, 900))
learning_curves + aes(alien_trial, rt, color = phase) + coord_cartesian(y = c(400, 900)) + facet_grid(paste("Alien", sad_alien) ~ paste("Task set", TS))  # Observation: RTs are slowest when performance is worst
...
# Rename item_chosen, item_left, and item_right columns: instead of "hot", "cold", and "rainy", call them "TS0", "TS1", and "TS2", depending on participant's randomization
...
for (row in 1:nrow(pick_aliens)) {
  if (!is.na(pick_aliens$item_chosen[row])) {
    if (nchar(pick_aliens$item_chosen[row]) == 1) {
      pick_aliens$item_chosen[row] = paste("TS", pick_aliens$item_chosen[row], sep = "")
    }
  }
  if (!is.na(pick_aliens$item_left[row])) {
    if (nchar(pick_aliens$item_left[row]) == 1) {
      pick_aliens$item_left[row] = paste("TS", pick_aliens$item_left[row], sep = "")
    }
  }
  if (!is.na(pick_aliens$item_right[row])) {
    if (nchar(pick_aliens$item_right[row]) == 1) {
      pick_aliens$item_right[row] = paste("TS", pick_aliens$item_right[row], sep = "")
    }
  }
}

# Add learnt values
alien_TS_values = ddply(learnt_values, .(subjID, sad_alien, TS), summarize,
                        av_reward = mean(reward, na.rm = T))
alien_TS_values$selected = with(alien_TS_values, paste("alien", as.numeric(as.character(sad_alien)) + 1, TS, sep = ""))
alien_TS_values$assess = "alien-same-TS"
alien_TS_values$sad_alien = alien_TS_values$TS = NULL
all_values = rbind(all_values, alien_TS_values)
...
ggplot(subset(pick_aliens, subjID < 15), aes(trial_index, value_diff, color = assess)) +
  geom_point() +
  geom_hline(yintercept = 0) +
  facet_grid(~ subjID)

pick_aliens_sum = ddply(subset(pick_aliens, !is.na(assess)), .(subjID, assess), summarize,
                        value_diff = mean(value_diff, na.rm = T))

ggplot(pick_aliens_sum, aes(assess, value_diff, color = assess)) +
...
ggplot(subset(comb_dat, subjID < 20), aes(sad_alien, item_chosen, color = TS, size = av_reward)) +
  geom_point() +
  theme(legend.position = "none") +
  facet_grid(paste("Subj", subjID) ~ paste("TS", TS))

ggplot(subset(comb_dat, TS == "rainbow"), aes(sad_alien, item_chosen, color = TS, size = av_reward)) +
  geom_point() +
  theme(legend.position = "none") +
...
comb_dat_sum$av_reward[comb_dat_sum$TS == "rainbow"] = 5 * comb_dat_sum$av_reward[comb_dat_sum$TS == "rainbow"] - 10  # to make differences in size more evident

ggplot(comb_dat_sum, aes(sad_alien, item_chosen, color = TS, size = av_reward)) +
  geom_point() +
  theme(legend.position = "none") +
