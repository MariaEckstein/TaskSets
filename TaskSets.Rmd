...
gg_value_guess = ggplot(feed_alien_trial1, aes(cum_ACC_, 100 * ACC)) +  # , color = cum_ACC_1back, group = cum_ACC_1back
...
if (gg_save) {
  ggsave(file.path(plot_dir, "/2gg_value_guess.png"), gg_value_guess, width = 10, height = 9)
}
```
...
gg_otherTS_ACC2 = gg_otherTS_ACC2 + facet_grid(phase ~ model_name)  # model_name does not show up for some reason..
...
# Run models
if (run_regression) {
  for (model_name in unique(dat_l$model_name)) {
...
subjID_phase_block_dat = ddply(subset(feed_aliens, phase != "5RainbowSeason"), .(subjID, phase, block), summarize, random = NA)
...
n_trials_long = data.frame(phase = feed_aliens_phases, n_trials = as.numeric(n_trials))
...
if (gg_save) {
  ggsave(file.path(plot_dir, "/6gg_alpha_values.png"), gg_alpha_values, width = 10, height = 8)
  ggsave(file.path(plot_dir, "/6gg_beta_values.png"), gg_beta_values, width = 10, height = 8)
}
```
```{r Start button}
```