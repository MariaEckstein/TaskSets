...
# TDs
# competition phase: check if RTs are faster for context-season and for season choices, because Q(s,a) and Q(c) are traditional RL things, and Q(a) and Q(s) are not.
...
if (run_on == "humans") {
  ...
  n_trials = data.frame(a = 13, b = 10, x = 13, d = 13)
  data_dir = "C:/Users/maria/MEGAsync/Berkeley/TaskSets/AlienGenRec/"
  patt = "sim_5...csv"  # "alien_..csv" -> sID1-9, "alien_...csv" -> sID10-99, "alien_....csv" -> sID100-999
  plot_dir = file.path(data_dir, "plots")
  simulated_keep_columns = c("sID", "trial_index", "phase", "TS", "context", "sad_alien", "item_chosen", "reward", "correct", "block.type", "trial_type", "learning_style", "mix_probs", "fit_pars", "RT", "Q_TS", "Q_action", "p_TS", "p_action")
}
...
for (filename in filenames) {
  ...
  if (run_on == "humans") {
    ...
    for (col in simulated_keep_columns) {
      if (! col %in% colnames(subj_file)) {
        subj_file[col] = NA
      }
    }
    # Subset important columns
    subj_file = subset(subj_file, select = simulated_keep_columns)
    # Rename "TS" column to "selected_TS"
    colnames(subj_file)[colnames(subj_file) == "TS"] = "selected_TS"
    # Rename "context" column to "TS"
    colnames(subj_file)[colnames(subj_file) == "context"] = "TS"
    # Add "season" column (with hot, cold, rainy)
    TS_seasons = data.frame(TS = 0:2, season = sample(c("hot", "cold", "rainy")))
    subj_file = merge(subj_file, TS_seasons, all.x = T)
    # Add sad_alien_name column
    alien_ids_names = data.frame(sad_alien = 0:3, sad_alien_name = sample(paste0("alien", 0:3)))
    subj_file = merge(subj_file, alien_ids_names, all.x = T)
    # Add item_chosen_name column
    item_ids_names = data.frame(item_chosen = 0:2, item_chosen_name = sample(c("bed", "plant", "umbrella")))
    subj_file = merge(subj_file, item_ids_names, all.x = T)
    # # Add phase column
    # if (is.na(subj_file$phase[0])) {
    #   n_trials_total = 0
    #   for (phas in paste0("X", feed_aliens_phases[feed_aliens_phases != "2CloudySeason"])) {
    #     n_trials_phase = as.numeric(n_seasons * n_aliens * n_blocks[phas] * n_trials[phas])
    #     subj_file$phase[subj_file$trial_index > n_trials_total] = gsub("X", "", phas)
    #     n_trials_total = n_trials_total + n_trials_phase
    #   }
    # }
  }
  ...
  for (row in 2:nrow(subj_file)) {
    if (is.na(subj_file$season[row])) {
      ...
      new_season_sim = (subj_file$season[row] != subj_file$season[row-1]) & !is.na(subj_file$season[row] != subj_file$season[row-1])
      new_season = ifelse(run_on != "simulations", is.na(subj_file$season[row-1]), new_season_sim)
      if (new_season) {
        trial = 1
      } else {
        trial = trial + 1
  ...
  subj_file = subj_file[order(subj_file$trial_index, subj_file$phase, subj_file$sad_alien, subj_file$TS),]
  ...
  if (run_on == "humans") {
    for (row in 1:nrow(subj_file)) {
      ...
      if (!is.na(sad_alien) & good_block_type) {  # trials in mixed blocks are not counted
        ...
        if (row > 1) {
          new_season = is.na(subj_file$season[row-1])
          if (new_season) {
    ...
    for (row in 1:nrow(subj_file)) {
      ...
      if (!is.na(sad_alien)) {
        ...
        if (new_season & !is.na(new_season)) {
          season_counter[season] = season_counter[season] + 1
        }
        subj_file$repetition[row] = season_counter[season][[1]]
        alien_season_counter[sad_alien, season] = alien_season_counter[sad_alien, season] + 1
...
all_files$phase = factor(all_files$phase)
all_files = subset(all_files, !is.na(phase))
...
if (run_on == "gen_rec") {
  overall_learning_curves =
    overall_learning_curves + aes(color = mix_probs, group = mix_probs) + facet_grid(~ learning_style)
}
if (gg_save) {
...
## TD FOR SIMULATED DATA: TS MEANS SOMETHING ELSE IN THESE DATA
