...
for (TSi in unique(rainbow_aliens$TS)) {
  ...
  rainbow_TS_w = merge(rainbow_TS_w,
                       ddply(rainbow_aliens, .(subjID), summarize,
                             n_valid_trials = sum(times_chosen)))
  rainbow_TS_w[TSi] = with(rainbow_TS_w, times_chosen_others.TRUE / n_valid_trials)
  rainbow_TS_w$times_chosen_others.FALSE = rainbow_TS_w$times_chosen_others.TRUE = rainbow_TS_w$n_valid_trials = NULL
...
rainbow_sum_l$TS = factor(rainbow_sum_l$TS, levels = c("No TS", "2", "1", "0"))
...
gg_rainbow_strategy = ggplot(rainbow_sum_l, aes(TS, Ratio, fill = TS)) + 
  ...
  geom_point(position = "jitter", alpha = 0.3) +
  labs(x = "", y = "TS choices / all choices", fill = "") +
  theme(legend.position = "none")

gg_rainbow_values = ggplot(subset(rainbow_aliens, round(av_reward) > 2),  # TD: TAKE OUT FOR NEW DATA!!!
       aes(round(av_reward), times_chosen, color = TS)) +
  stat_summary(fun.data = mean_se, geom = "pointrange") +
  labs(x = "Value", y = "Times chosen", color = "TS")
