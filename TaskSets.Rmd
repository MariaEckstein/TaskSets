...
# TDs
# Make sure everything has model_name -> every plot has facets!
...
if (run_on == "humans") {
  ...
  data_dir = "C:/Users/maria/MEGAsync/Berkeley/TaskSets/AlienGenRec/allFlatAgents/"
...
for (subjIDi in unique(all_files$subjID)) {
  ...
  subj_model_parameters = paste(parameter_names[all_files[subj_rows, parameter_names][1,] != 0], collapse="_")[1]
...
```
```{r Get simulated data into the right format - remove excess columns and rows}
...
if (gg_save) {
  ggsave(file.path(plot_dir, "1overall_learning_curves.png"), overall_learning_curves, width = 20, height = 8)
}
```
```{r demographics}
...
if (gg_save) {
  ...
  ggsave(file.path(plot_dir, "4gg_pick_aliens_raw_ch.png"), gg_pick_aliens_raw_ch, width = 8, height = 10)
  ggsave(file.path(plot_dir, "4gg_pick_aliens_raw_unch.png"), gg_pick_aliens_raw_unch, width = 8, height = 10)
  ggsave(file.path(plot_dir, "4gg_pick_aliens_raw_diff.png"), gg_pick_aliens_raw_diff, width = 8, height = 10)
  
  ggsave(file.path(plot_dir, "4gg_pick_aliens_value_subj.png"), gg_pick_aliens_value_subj, width = 4, height = 10)
  ggsave(file.path(plot_dir, "4gg_pick_aliens_value_obj.png"), gg_pick_aliens_value_obj, width = 4, height = 10)
  ggsave(file.path(plot_dir, "4gg_pick_aliens_ACC.png"), gg_pick_aliens_ACC, width = 4, height = 10)
  
  ggsave(file.path(plot_dir, "4gg_pick_aliens_perc_subj.png"), gg_pick_aliens_perc_subj, width = 4, height = 10)
  ggsave(file.path(plot_dir, "4gg_pick_aliens_perc_obj.png"), gg_pick_aliens_perc_obj, width = 4, height = 10)
}
}
```
...
if (gg_save) {
  ggsave(file.path(plot_dir, "0value_plot_ids.png"), value_plot_ids, width = 9, height = 9)
  ggsave(file.path(plot_dir, "0value_plot_ids_obj.png"), value_plot_ids_obj, width = 9, height = 9)
  # ggsave(file.path(plot_dir, "0value_plot_ids_subj.png"), value_plot_ids_subj, width = 15, height = 15)
  ggsave(file.path(plot_dir, "0value_plot_names.png"), value_plot_names, width = 9, height = 9)
  ggsave(file.path(plot_dir, "0TS_plot.png"), TS_plot, width = 9, height = 9)
  ggsave(file.path(plot_dir, "0TS_plot_obj.png"), TS_plot_obj, width = 9, height = 9)
...
if (gg_save) {
  ggsave(file.path(plot_dir, "/2gg_subj_obj_values.png"), gg_subj_obj_values, width = 3, height = 7)
  
  ggsave(file.path(plot_dir, "/2gg_learning_curves_TS_value_obj.png"), gg_learning_curves_TS_value_obj, width = 10, height = 10)
  ggsave(file.path(plot_dir, "/2gg_learning_curves_TS_value_subj.png"), gg_learning_curves_TS_value_subj, width = 10, height = 10)
  ggsave(file.path(plot_dir, "/2gg_learning_curves_item_value_obj.png"), gg_learning_curves_item_value_obj, width = 10, height = 10)
  ggsave(file.path(plot_dir, "/2gg_learning_curves_item_value_subj.png"), gg_learning_curves_item_value_subj, width = 10, height = 10)
...
if (gg_save) {
  ggsave(file.path(plot_dir, "/2gg_prev_ACC.png"), gg_prev_ACC, width = 10, height = 9)
  ggsave(file.path(plot_dir, "/2gg_prev_seen.png"), gg_prev_seen, width = 10, height = 9)
}
```
...
for (subj in unique(feed_aliens$subjID)) {
  for (blocki in unique(subset(feed_aliens, subjID == subj)$block)) {
    ...
    current_TS = feed_aliens[subj_block_sub, "TS"][1]
...
if (gg_save) {
  ...
  ggsave(file.path(plot_dir, "/2gg_otherTS_ACC3.png"), gg_otherTS_ACC3, width = 10, height = 10)
  ggsave(file.path(plot_dir, "/2gg_otherTS_ACC4.png"), gg_otherTS_ACC4, width = 5, height = 10)
  ...
  ggsave(file.path(plot_dir, "/2gg_prevTS_ACC2.png"), gg_prevTS_ACC2, width = 10, height = 10)
}
```
```{r Run perseverance regression models}

subjwise_regr_coefs = expand.grid(subjID = unique(all_files$subjID), phase = feed_aliens_phases)
intrusion_predictors = c("TS_value_obj", "item_value_obj", "TS_value_subj", "item_value_subj")
for (predictor in intrusion_predictors) {
  subjwise_regr_coefs[,predictor] = NA
}
...
for (subj in unique(ACC_1back_dat$subjID)) {
  for (phas in feed_aliens_phases) {
    ...
    intrusion_formula = paste("ACC ~", paste(intrusion_predictors, collapse = " + "), "+", paste(predictors_no_interest, collapse = " + "))
    
    intrusion_mod = glm(as.formula(intrusion_formula),
        family = binomial,
        data = dat)
    intrusion_coefs = as.data.frame(summary(intrusion_mod)$coef)
    
    for (predictor in intrusion_predictors) {
      subjwise_regr_coefs[subjwise_regr_coefs$subjID == subj & subjwise_regr_coefs$phase == phas, paste0("intrusion", predictor)] = intrusion_coefs[predictor, "Estimate"]
    }
  }
}
...
if (run_regression) {
  for (model_name in unique(persev_dat$model_name)) {
    for (phas in unique(persev_dat$phase)) {  # I could also just run one single regression model and include phase as a predictor
      ...
      write.csv(as.data.frame(summary(glmer_mod_intrusions)$coef), paste(plot_dir, "/glmer_mod_intrusions_summary_", phas, "_", model_namei, ".csv", sep = ""))
    }
  }
}
...
ggsave(file.path(plot_dir, "3regression_plot.png"), regression_plot, width = 10, height = 10)

# subjwise_regr_coefs = expand.grid(subjID = unique(all_files$subjID), phase = feed_aliens_phases)
value_predictors = c("TS_value_obj", "item_value_obj", "TS_value_subj", "item_value_subj")
reactivation_predictors = c("ACC_1back_self", "ACC_1back_others")
for (predictor in c(value_predictors, reactivation_predictors)) {
  subjwise_regr_coefs[,predictor] = NA
}
...
for (subj in unique(ACC_1back_dat$subjID)) {
  for (phas in feed_aliens_phases) {
    ...
    predictors_no_interest = c("repetition", "alien_trial_sum")
    value_formula = paste("ACC ~", paste(value_predictors, collapse = " + "), "+", paste(predictors_no_interest, collapse = " + "))
    reactivation_formula = paste("ACC ~", paste(reactivation_predictors, collapse = " + "), "+", paste(predictors_no_interest, collapse = " + "))
    
    value_mod = glm(as.formula(value_formula),
        family = binomial,
        data = dat)
    value_coefs = as.data.frame(summary(value_mod)$coef)
    
    reactivation_mod = update(value_mod, as.formula(reactivation_formula))
    reactivation_coefs = as.data.frame(summary(reactivation_mod)$coef)
    
    for (predictor in value_predictors) {
      subjwise_regr_coefs[subjwise_regr_coefs$subjID == subj & subjwise_regr_coefs$phase == phas, predictor] = value_coefs[predictor, "Estimate"]
    }
    for (predictor in reactivation_predictors) {
      subjwise_regr_coefs[subjwise_regr_coefs$subjID == subj & subjwise_regr_coefs$phase == phas, predictor] = reactivation_coefs[predictor, "Estimate"]
    }
  }
}
  ...
  for (model_namei in unique(ACC_1back_dat$model_name)) {
    for (phas in feed_aliens_phases) {
      ...
      glmer_mod_trial_value_obj = glmer(as.formula(trial_value_form_obj),
                        ...
                        na.action = na.omit,
                        control = glmerControl(optimizer = 'bobyqa', optCtrl = list(maxfun = 1e6)))
      write.csv(as.data.frame(summary(glmer_mod_trial_value_obj)$coef), paste(plot_dir, "/glmer_trial_value_obj_", phas, "_", model_namei, ".csv", sep = ""))
      
      glmer_mod_trial_value_subj = update(glmer_mod_trial_value_obj, formula = as.formula(trial_value_form_subj))
      write.csv(as.data.frame(summary(glmer_mod_trial_value_subj)$coef), paste(plot_dir, "/glmer_trial_value_subj_", phas, "_", model_namei, ".csv", sep = ""))
          
      glmer_mod_1back_self_others = update(glmer_mod_trial_value_obj, formula = as.formula(oneback_self_others_form))
      write.csv(as.data.frame(summary(glmer_mod_1back_self_others)$coef), paste(plot_dir, "/glmer_oneback_self_others_summary_", phas, "_", model_namei, ".csv", sep = ""))
      
      glmer_mod_rew_1back = update(glmer_mod_trial_value_obj, formula = as.formula(rew_1back_form))
      write.csv(as.data.frame(summary(glmer_mod_rew_1back)$coef), paste(plot_dir, "/glmer_rew_1back_summary_", phas, "_", model_namei, ".csv", sep = ""))
    }
  }
}
```
...
if (gg_save) {
  ggsave(file.path(plot_dir, "5gg_rainbow_avs.png"), gg_rainbow_avs, width = 5, height = 8)
  
  ggsave(file.path(plot_dir, "5gg_rainbow_TS.png"), gg_rainbow_TS, width = 5, height = 8)
  ggsave(file.path(plot_dir, "5gg_rainbow_TS_full.png"), gg_rainbow_TS_full, width = 5, height = 8)
  
  ggsave(file.path(plot_dir, "5gg_rainbow_value_obj.png"), gg_rainbow_value_obj, width = 5, height = 8)
  ggsave(file.path(plot_dir, "5gg_rainbow_value_subj.png"), gg_rainbow_value_subj, width = 5, height = 8)
  ggsave(file.path(plot_dir, "5gg_rainbow_ACC.png"), gg_rainbow_ACC, width = 5, height = 8)
}
```
...
feed_aliens$rolling_average_ACC = rollmean(feed_aliens$ACC, 8, fill = NA, align = "right")
feed_aliens$rolling_average_ACC[feed_aliens$alien_trial_sum < 3] = NA  # the first 8 trials (2 x 4 aliens) of each block need to be NA; otherwise they carry over between blocks / between subjects
subset(feed_aliens, select = c("subjID", "sad_alien", "trial_index", "alien_trial_sum", "TS", "ACC", "rolling_average_ACC"))

# Check that it looks good
ggplot(subset(feed_aliens, subjID == 500 & trial_index < 500), aes(trial_index, rolling_average_ACC, color = block %% 3)) +
  geom_point() +
  geom_hline(yintercept = 6/12, linetype = "dotted") +
...
six_of_8_correct_dat = ddply(subset(feed_aliens, reached_6_out_of_8), .(subjID, phase, block), summarize,
                                 trial_reached = min(alien_trial_sum))
subjID_phase_block_dat = ddply(subset(feed_aliens, phase != "5RainbowSeason"), .(subjID, phase, block), summarize, random = NA)  # which block was part of which season for which agent?
six_of_8_correct_dat = merge(six_of_8_correct_dat, subset(subjID_phase_block_dat, select = -random), by = c("subjID", "phase", "block"), all.y = T)  # add blocks that are missing because agent never reached criterion
n_trials_long = data.frame(phase = feed_aliens_phases, n_trials = as.numeric(n_trials))  # how many trials are presented in each phase (per alien)?
six_of_8_correct_dat = merge(six_of_8_correct_dat, n_trials_long, by = "phase", all.x = T)  # add number of trials for each phase
six_of_8_correct_dat$trial_reached[is.na(six_of_8_correct_dat$trial_reached)] = six_of_8_correct_dat$n_trials[is.na(six_of_8_correct_dat$trial_reached)] + 1  # set trials_to_criterion to the maximum number of trials in this phase + 1 for blocks in which agents never reached criterion
...
gg_alpha_6_of_8 = ggplot(six_of_8_correct_dat, aes(round(alpha, 1), trial_reached, color = phase)) +
  stat_summary(fun.data = "mean_se", geom = "pointrange") +
  stat_summary(fun.data = "mean_se", geom = "line") +
  labs(x = "Alpha (binned)", y = "Trials to criterion after switch") +
  facet_grid(model_name ~ .)
gg_beta_6_of_8 = gg_alpha_6_of_8 + aes(x = round(beta)) + labs(x = "Beta (binned)")
gg_forget_6_of_8 = gg_alpha_6_of_8 + aes(x = 2 * round(forget / 2, 2)) + labs(x = "Forget (binned)")

if (gg_save) {
  ggsave(file.path(plot_dir, "/6gg_alpha_six_of_8.png"), gg_alpha_6_of_8, width = 4, height = 8)
  ggsave(file.path(plot_dir, "/6gg_beta_six_of_8.png"), gg_beta_6_of_8, width = 4, height = 8)
  ggsave(file.path(plot_dir, "/6gg_forget_six_of_8.png"), gg_forget_6_of_8, width = 4, height = 8)
}
```
...
subjwise_regr_coefs_long = melt(subset(subjwise_regr_coefs), id.vars = c("subjID", "model_name", "phase", parameter_names), variable.name = "factor", value.name = "regr_beta")
beta_limits = quantile(subjwise_regr_coefs_long$regr_beta, c(.1, .9), na.rm = T)
subjwise_regr_coefs_long = subset(subjwise_regr_coefs_long, regr_beta > beta_limits[1] & regr_beta < beta_limits[2])

gg_alpha_values = ggplot(subjwise_regr_coefs_long, aes(round(alpha, 1), regr_beta, color = phase, group = phase)) +
  stat_summary(fun.data = "mean_se", geom = "pointrange") +
  geom_hline(yintercept = 0, linetype = "dotted") +
  labs(x = "Alpha (binned)", y = "Effect on ACC (regr. beta)") +
  facet_grid(model_name ~ factor)
gg_beta_values = gg_alpha_values + aes(x = round(beta)) + labs(x = "Beta (binned)")

if (gg_save) {
  ggsave(file.path(plot_dir, "/6gg_alpha_values.png"), gg_alpha_values, width = 20, height = 8)
  ggsave(file.path(plot_dir, "/6gg_beta_values.png"), gg_beta_values, width = 20, height = 8)
}
```
```{r Start button}
```