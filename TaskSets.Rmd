...
# TDs
# check if participants reactivte TS in rainbow phase, or just individual associations
...
t.test(subset(pick_aliens_sum, assess == "item")$selected_better - subset(pick_aliens_sum, assess == "TS")$selected_better)
...
regr_dat = subset(feed_aliens, select = c("subjID", "phase", "sad_alien", "TS", "reward", "av_reward", "TS_value", "trial", "alien_trial_sum", "alien_trial", "ACC"))
...
for (phas in feed_aliens_phases) {
  ...
  if (run_regression) {
    ...
    # others$ACC_TS = factor(others$ACC_TS, levels = c("None", "ACC0", "ACC1", "ACC2"), labels = c(0, 6.25, 5, 3.75))
    others$ACC_TS = as.numeric(as.character(others$ACC_TS))
    intrusions_form = "ACC ~ alien_trial_sum + ACC_TS + av_reward_ACC_TS + (1 | subjID)"
    ...
    write.csv(as.data.frame(anova(glmer_mod_intrusions, type = 3)), paste(plot_dir, "/glmer_mod_intrusions_aov_", phas, ".csv", sep = ""))
  }
...
if (run_regression) {
  for (phas in feed_aliens_phases) {
    ...
    trial_value_form = "ACC ~ TS_value + av_reward + alien_trial_sum + (1 | subjID)"  # I can't add TS_value:av_reward because that will make the models for refresher2 and refresher3 crash
    
    glmer_mod_trial_value = glmer(as.formula(trial_value_form),
                      family = binomial,
                      data = subset(regr_dat, phase == phas & TS != 2),
                      na.action = na.omit,
                      control = glmerControl(optimizer = 'bobyqa', optCtrl = list(maxfun = 1e6)))
    write.csv(as.data.frame(summary(glmer_mod_trial_value)$coef), paste(plot_dir, "/glmer_trial_value_summary_", phas, ".csv", sep = ""))
    write.csv(as.data.frame(anova(glmer_mod_trial_value, type = 3)), paste(plot_dir, "/glmer_trial_value_aov_", phas, ".csv", sep = ""))
    ...
    # glmer_mod_rew_1back_value = update(glmer_mod_1back_self_others, formula = as.formula(rew_1back_value_form))
    # write.csv(as.data.frame(summary(glmer_mod_rew_1back_value)$coef), paste(plot_dir, "/glmer_rew_1back_value_summary_", phase, ".csv", sep = ""))
    # write.csv(as.data.frame(anova(glmer_mod_rew_1back_value, type = 3)), paste(plot_dir, "/glmer_rew_1back_value_aov_", phase, ".csv", sep = ""))
...
cloudy_sum = ddply(cloudy_aliens, .(sad_alien, subjID), summarize,
...
cloudy_sum_l = melt(cloudy_sum, id.vars = c("subjID", "sad_alien"), variable.name = "TS", value.name = "times_chosen")
