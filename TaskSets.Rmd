...
for (TSi in unique(correct_TS$TS)) {
  feed_aliens = merge(feed_aliens, subset(correct_TS, TS == TSi, select = -TS),
        by.y = c("sad_alien", "item_chosen"),
        by.x = c("sad_alien", "item_chosen"),
        all.x = T,
        suffixes = c("", TSi))
  col = paste("ACC", TSi, sep = "")
  feed_aliens[col][is.na(feed_aliens[col])] = 0
}
...
for (phas in c("1InitialLearning", "2CloudySeason")) {
  ...
  dat_l = melt(subset(dat, select = -ACC), measure.vars = c("ACC0", "ACC1", "ACC2"), variable.name = "ACC_TS", value.name = "ACC")
  dat_l$facet = ""
  gg_otherTS_ACC = ggplot(dat_l, aes(alien_trial, 100 * ACC, color = factor(ACC_TS))) +
    stat_summary(fun.data = mean_se, geom = "pointrange") +
    geom_hline(yintercept = 100/3, linetype = "dotted") +
    coord_cartesian(y = c(0, 100)) +
    labs(x = "Trial", y = "% correct", color = "") +
    facet_grid(~ TS)
  ...
  dat_l2 = melt(dat, measure.vars = c("ACC", "ACC_prev_TS"), variable.name = "ACC_TS", value.name = "ACC")  # , "ACC_prev_prev_TS"
  dat_l2$facet = ""
  ...
  gg_prevTS_ACC2 = gg_prevTS_ACC + aes(color = ACC_TS, group = ACC_TS) + facet_grid(~ facet) + labs(color = "")
  gg_prevTS_ACC2$data = dat_l2
  
  if (gg_save) {
    ggsave(paste(plot_dir, "/2gg_otherTS_ACC_", phas, ".png", sep = ""), gg_otherTS_ACC, width = 9, height = 3)
    ggsave(paste(plot_dir, "/2gg_otherTS_ACC_value_", phas, ".png", sep = ""), gg_otherTS_ACC_value, width = 9, height = 7)
    ggsave(paste(plot_dir, "/2gg_otherTS_ACC_alien_", phas, ".png", sep = ""), gg_otherTS_ACC_alien, width = 9, height = 7)
    ...
    ggsave(paste(plot_dir, "/2gg_otherTS_ACC_value2_", phas, ".png", sep = ""), gg_otherTS_ACC_value2, width = 9, height = 7)
    ggsave(paste(plot_dir, "/2gg_otherTS_ACC_alien2_", phas, ".png", sep = ""), gg_otherTS_ACC_alien2, width = 9, height = 7)
    ggsave(paste(plot_dir, "/2gg_prevTS_ACC2_", phas, ".png", sep = ""), gg_prevTS_ACC2, width = 4, height = 2)
  }
}
```
...
rainbow_aliens = ddply(subset(feed_aliens, phase == "5RainbowSeason" & !is.na(key) & !is.na(rt)),
                       .(subjID, sad_alien, TS, item_chosen),
                       summarize,
                       times_chosen = length(item_chosen))  # in how many trials subject chose each item for each alien
rainbow_aliens = merge(subset(rainbow_aliens, select = -TS), correct_values,  # what's the TS that the choice corresponds to?
...
rainbow_sum_l = melt(rainbow_sum, id.vars = "subjID", variable.name = "TS", value.name = "Ratio")
rainbow_sum_l$Ratio[is.na(rainbow_sum_l$Ratio)] = 0
rainbow_sum_l$TS = factor(rainbow_sum_l$TS, levels = c("No TS", "2", "1", "0"), labels = c("No TS", "TS2", "TS1", "TS0"))

gg_rainbow_strategy = ggplot(rainbow_sum_l, aes(TS, Ratio, fill = TS)) + 
...
gg_rainbow_values = ggplot(subset(rainbow_aliens, round(av_reward) > 2),  # TD: TAKE OUT FOR NEW DATA!!!
       aes(round(av_reward), times_chosen / 3, color = TS)) +
  stat_summary(fun.data = mean_se, geom = "pointrange") +
...
if (gg_save) {
  ...
  ggsave(file.path(plot_dir, "5gg_rainbow_values.png"), gg_rainbow_values, width = 5, height = 3)
}
```
