...
library("ggplot2"); library("plyr"); library("lme4"); library("reshape2"); library("zoo"); library("lmerTest")
theme_set(theme_bw())

# TDs
# change colnames(all_files)[colnames(all_files) == "create_TS_biased_prefer_new"] = "create_TS_biased"

run_on = "humans"  # can be "fitted_humans", "gen_rec", "humans", or "simulations"
gg_save = T
run_regression = F
parameter_names = c("alpha", "beta", "forget", "alpha_high", "beta_high", "forget_high")  # , "beta_high", 'TS_bias', "epsilon"
...
wanted_columns = c("subjID", "trial_index", "phase", "TS", "TS_chosen", "season", "sad_alien", "sad_alien_name", "item_left", "item_right", "item_chosen", "item_left_name", "item_right_name", "item_chosen_name", "assess", "reward", "correct", "block.type", "block", "trial_type", "model_name", "rt", "Q_TS", "Q_action", "p_TS", "p_action", "LL")
...
if (run_on == "humans") {
  data_dir = "C:/Users/maria/MEGAsync/Berkeley/TaskSets/Data/version1.0"  # version3.1  # versions1.0and3.1  # version1.0  # versionEEG
  patt = ".*csv"
} else if (run_on == "simulations") {
  data_dir = "C:/Users/maria/MEGAsync/Berkeley/TaskSets/AliensFitting/SummariesInsteadOfFitting_revision/FullSimulations/" # "C:/Users/maria/MEGAsync/SLCN/PSsimulations/"   # "C:/Users/maria/MEGAsync/Berkeley/TaskSets/AlienGenRec/MaxInsteadOfSoftmax/"
  patt = "aliens_flat.*csv"  # "alien_..csv" -> subjID1-9, "alien_...csv" -> subjID10-99, "alien_....csv" -> subjID100-999
  feed_aliens_phases = feed_aliens_phases[feed_aliens_phases != "2CloudySeason"]
} else if (run_on == "gen_rec") {
  data_dir = "C:/Users/maria/MEGAsync/Berkeley/TaskSets/AlienGenRec/fit_par/"
  patt = "sim_....csv"  # "alien_..csv" -> subjID1-9, "alien_...csv" -> subjID10-99, "alien_....csv" -> subjID100-999
  # parameter_names = c(parameter_names, paste0(parameter_names, "_rec"))
}
...
perf_criterion = 0.35  # participants who perform worse that this during the whole IL phase will be excluded
```
```{r Read in all data}
...
for (filename in filenames) {
  ...
  subj_file$X = NULL
  subj_file$correct = factor(subj_file$correct)  # necessary because otherwise correct == NA everywhere for subject 163
  ...
  if (run_on == "humans") {
    ...
    for (wanted_column in wanted_columns) {
      if (! wanted_column %in% colnames(subj_file)) {
        subj_file[wanted_column] = NA
      }
    }
    subj_file = subset(subj_file, select = c(wanted_columns, "key"))  # necessary to read in version1.0 and version 3.1 human data together
  }
...
subj_file
all_files
# unique(all_files$correct)
summary(all_files)
dim(all_files)
orig_all_files = all_files
unique(all_files$subjID)
# chance_performers
```
```{r Human demographics and training trials}
if (run_on == "humans") {
  train_dat = subset(all_files, phase == "0Training")
}
...
old_allfiles = all_files

if (run_on %in% c("simulations", "gen_rec")) {
  all_files[all_files$model_name %in% c("flat", "s-flat", "Bayes"), "alpha_high"] = 0
  all_files[all_files$model_name %in% c("flat", "s-flat"), "beta_high"] = 0
  all_files[all_files$model_name == "hier" & all_files$alpha == all_files$alpha_high, "alpha_high"] = 0
  all_files[all_files$model_name == "hier" & all_files$beta == all_files$beta_high, "beta_high"] = 0
  # Rename TS-related columns
  colnames(all_files)[colnames(all_files) == "context"] = "TS"
  
  # Remove unwanted rows
  if (!"phase" %in% colnames(all_files)) {
    all_files$phase = "1InitialLearning"
  }
  subj_value_phase = "1InitialLearning"
  all_files = subset(all_files, reward != 0)
  ...
  for (subjIDi in unique(all_files$subjID)) {
    ...
    # all_files[subj_rows, "model_name"] = paste(non0_parameters, collapse="_")[1]
    ...
    if (run_on == "simulations") {
      ...
      subj_file_feed$TS = as.numeric(as.character(subj_file_feed$TS))
      ...
      subj_file_feed = merge(subj_file_feed, item_ids_names, by.x = "item_chosen", by.y = "item", all.x = T)
      colnames(subj_file_feed)[colnames(subj_file_feed) == "item_name"] = "item_chosen_name"  # paste0(item, "_name")
...
```{r Get parameter_dat}
if (run_on == "gen_rec") {
  parameter_dat = ddply(all_files, c("subjID", "model_name", "NLL_rec", parameter_names, paste0(parameter_names, "_rec")), summarize,
                        NLL = -LL[trial_index == max(trial_index)])  # NLL in the last trial of InitialLearning
  parameter_long_gen = melt(subset(parameter_dat, select = c(parameter_names, "NLL", "subjID", "model_name")),
                            measure.vars = c(parameter_names, "NLL"), variable.name = "parameter", value.name = "gen_par")
  parameter_long_rec = melt(subset(parameter_dat, select = c(paste0(parameter_names, "_rec"), "NLL_rec", "subjID", "model_name")),
  ...
  parameter_long = merge(parameter_long_gen, parameter_long_rec, by = c("subjID", "model_name", "parameter"))
  assertthat::assert_that(dim(parameter_long)[1] == (length(parameter_names) + 1) * dim(parameter_dat)[1])
  
} else if (run_on == "simulations") {
  parameter_dat = ddply(all_files, c("subjID", "model_name", parameter_names), summarize, bla=NA)
  parameter_dat$bla = NULL
...
```{r plot generated against recovered parameters & NLL}
if (run_on == "gen_rec") {
  ...
  for (par_name in unique(parameter_long$parameter)) {
    ...
    outliers = c(outliers, subset(parameter_long, par_rows & (abs(gen_minus_rec) > cutoff))$subjID)
  }
  parameter_long$outlier = parameter_long$subjID %in% outliers
  
  # Plot
  gg_gr =
    ggplot(subset(parameter_long, gen_par != 0), aes(gen_par, rec_par - gen_par, color = outlier)) +
    geom_point() +
    geom_text(aes(label = ifelse(outlier, subjID, ""), hjust = 0)) +
...
# Finish all_files preprocessing
...
all_files
subset(all_files, select = c("subjID", "TS", "TS_chosen", "correct", "sad_alien", "reward", "item_chosen"))
```
...
```
```{r Add columns block, alien_trial, and repetition}
...
for (subjIDi in unique(all_files$subjID)) {
  ...
  for (row in 1:nrow(subj_file)) {
    ...
    if (row == 1) {
      ...
      new_phase = subj_file$phase[row] != subj_file$phase[row-1]
    }
    
    if (!is.na(sad_alien) & !is.na(season)) {
      # Update season and season-alien counters
      if (new_season | new_phase) {
...
all_files$ACC = ifelse(as.character(all_files$correct) %in% c("true", "True", T, 1, "1"), 1, 0)
```
...
# Make sure that participants are rewarded according to a legit TS in one particular context
ddply(subset(all_files, season == "hot" & ACC == 1), .(subjID, TS, sad_alien, item_chosen), summarize,
      rew = mean(reward, na.rm = T))
```
```{r Save preprocessed subj_files}
# print(paste0("Saving preprocessed files to ", data_dir))
# for (subj in unique(all_files$subjID)) {
#   write.csv(subset(all_files, subjID == subj), paste0(data_dir, "preprocessed/alien", subj, ".csv"))
# }
```
```{r Exclude subjects who don't reach criterion in training block and plot performance and missed trials}
if (run_on %in% c("humans", "simulations")) {
  
  ggplot(subset(all_files, phase=="1InitialLearning"), aes(subjID, ACC, color=TS)) +
    stat_summary(fun.data=mean_cl_normal, geom='pointrange')
  
  perf_dat = ddply(subset(all_files, phase=="1InitialLearning"),
                   .(subjID), summarize,
                   ACC = mean(correct %in% c("true", "TRUE", T), na.rm = T))
  ...
  missed_trials_dat = ddply(subset(all_files, !is.na(phase)), .(subjID, phase, block.type), summarize,
        n_missed_trials = sum(is.na(item_chosen)),
        perc_missed_trials = 100 * mean(is.na(item_chosen)),
...
# Create NIH data summary

```{r}
# demos = read.csv("C:/Users/maria/MEGAsync/Berkeley/TaskSets/Data/DemographicsEEG.csv")
# demos = subset(demos, select=c("Subject.ID", "Pseudo.GUID"))
# demos$subjID = demos$Subject.ID
# demos$Subject.ID = NULL
# 
# nih_sum = ddply(all_files, .(subjID, TS, phase), reward=mean(reward, na.rm=T), ACC=mean(ACC, na.rm=T), RT=median(rt, na.rm=T), summarize)
# nih_sum = subset(nih_sum, !is.na(phase))
# nih_sum$reward[nih_sum$phase == "5RainbowSeason"] = NA
# nih_sum$ACC[nih_sum$phase == "5RainbowSeason"] = NA
# 
# nih_sum = merge(nih_sum, demos, all.x=T, all.y=F)
# write.csv(nih_sum, paste0(data_dir, "nih_summary.csv"))
```
...
ggplot(all_files, aes(trial_index, ACC, color = phase, group = phase)) +
  stat_summary(fun.data = mean_se)

# Plot learning curves (and RTs) overall
overall_learning_curves =
  ggplot(all_files, aes(trial_index, 100 * ACC, color = phase, group = phase)) +
    stat_summary(fun.data = mean_se, geom = "pointrange") +
...
dat = subset(all_files, phase %in% c("1InitialLearning", "Refresher2", "Refresher3") & (!is.na(item_chosen_name) | !is.na(item_chosen)) & (is.na(block.type) | block.type != "mixed"))
...
all_files
subset(dat, subjID > 100)  # make sure version3.1 data is there
subset(dat, subjID < 100)  # make sure version1.0 and versionEEG data is there
```
...
if (!run_on %in% c("gen_rec", "simulations")) {
  ...
  pick_aliens$selected_better_obj[pick_aliens$value_obj_unchosen == pick_aliens$value_obj_chosen] = NaN
  ...
  if (substr(data_dir, 55, 58) != "1.0") {
    ...
    pick_aliens_counts_chosen_names = ddply(subset(pick_aliens),
                                            .(subjID, assess, item_chosen_name, model_name), summarize,
                                            count_chosen = length(item_chosen_name))
    pick_aliens_counts_unchosen_names = ddply(subset(pick_aliens),
                                              .(subjID, assess, item_unchosen_name, model_name), summarize,
                                              count_unchosen = length(item_unchosen_name))
  ...
  print(paste0("Saving preprocessed files to ", data_dir))
  for (subj in unique(all_files$subjID)) {
    write.csv(subset(pick_aliens, subjID == subj), paste0(data_dir, "preprocessed/pick_alien", subj, ".csv"))
  }
}
```
...
# Remove mixed blocks
feed_aliens = subset(all_files, trial_type == "feed-aliens" & (is.na(block.type) | block.type != "mixed"))
...
### Add data for subj 9 (dropped out before refresher 2)
if (substr(data_dir, 55, 57) == "EEG") {
  subj9 = subset(all_values_perf_sub, subjID == 8)
  subj9$subjID = 9
  subj9$item_value_subj = NA
  all_values_perf_sub = rbind(all_values_perf_sub, subj9)
}
...
all_values_perf_sub[order(all_values_perf_sub$subjID),]

### Add data for subj 9 (dropped out before refresher 2)
if (substr(data_dir, 55, 57) == "EEG") {
  subj9 = subset(all_values_perf_sub, subjID == 8)
  subj9$subjID = 9
  subj9$TS_value_subj = NA
  subj9$av_ACC = NA
  all_values_perf_sub = rbind(all_values_perf_sub, subj9)
}

### Add subj and obj values to feed_aliens
feed_aliens = merge(feed_aliens, all.x = T,
...
# Show feed_aliens
feed_aliens
subset(ddply(feed_aliens, .(subjID, phase, TS, TS_value_obj, item_value_obj), summarize, ACC=mean(ACC)), subjID > 100)
subset(all_values_perf, subjID > 100)
subset(feed_aliens, subjID==0 & sad_alien==1 & TS==0)
```

```{r}
subset(feed_aliens, phase == "1InitialLearning" & is.na(TS_value_obj))
```
```{r Plots}
...
gg_learning_curves_TS_value_obj =
  ggplot(subset(feed_aliens, phase != "0Training"),
         aes(alien_trial_sum, 100 * ACC, color = TS_value_obj, group = TS_value_obj)) +
  stat_summary(fun.data = mean_se, geom = "pointrange") +
...
# feed_aliens$repetition = as.numeric(as.character(feed_aliens$repetition))
...
gg_learning_curves_rep$data = subset(feed_aliens, phase == '1InitialLearning')
...
if (gg_save) {
  ...
  ggsave(file.path(plot_dir, "/2gg_learning_curves_rep.png"), gg_learning_curves_rep, width = 4, height = 4)
  ggsave(file.path(plot_dir, "/2gg_learning_curves_wide.png"), gg_learning_curves_wide, width = 10, height = 5)
}

nrow(ddply(subset(feed_aliens, phase != "0Training"), .(subjID), ACC=mean(correct=="true", na.rm=T), summarize))  # check that we are using the rigth data (version1.0 -> 51 participants; version3.1 -> 31 participants)
(subset(feed_aliens, !phase %in%c("0Training") & is.na(TS_value_obj)))
summary(feed_aliens)
```
...
# Add n_seen_aliens and n-back columns (I wish there was a better way to do this, with rollmean or so, but rollmean sets everything before to NA..)
    ...
    if (sum(sub_subj_block) == 4) {
      ...
      for (n in 1:(n_aliens-1)) {
        ...
        if (mean(sub_subj_block) > 0) {
          ...
          feed_alien_trial1[sub_subj_block, col] = ACC_aliens_before[1:sum(sub_subj_block)]
        }
      }
    }
  }
...
gg_otherTS_ACC3 = ggplot(subset(persev_dat, current_TS == F), aes(alien_trial_sum, 100 * ACC, color = ACC_TS)) +
  stat_summary(fun.data = mean_se, geom = "pointrange") +
  labs(x = "", y = "% intrusion errors", color = "", shape = "") +
  facet_grid(model_name ~ phase)
...
```
```{r Run 1back_self 1back_other regression models}
...
for (subj in unique(ACC_1back_dat$subjID)) {
  print(paste("Calculating SubjID", subj, "(", length(unique(ACC_1back_dat$subjID)), "subjects total)"))
  ...
  for (phas in feed_aliens_phases) {
    ...
    if (nrow(dat) == 0) {
      print(paste("Subject", subj, "phase", phas, "is missing!"))
      run_this_regr = F
    } else {
    
      for (predictor in c(value_obj_predictors, nback_predictors, predictors_no_interest)) {
        pred_dat = dat[,predictor]
        zscored_pred = (pred_dat - mean(pred_dat, na.rm = T)) / sd(pred_dat, na.rm = T)
        dat[,paste0(predictor, "_zscored")] = zscored_pred
        if (mean(is.na(zscored_pred)) == 1) {
          run_this_regr = F
        }
      }
    }
...
```
```{r Plot subjwise regression results}
interesting_predictors = paste0(c("item_value_obj", "TS_value_obj", "ACC_1back_self", "ACC_1back_others", "rew_1back_self"), "_zscored")

gg_subjwise_regr =
  ggplot(subjwise_regr_coefs, aes(predictor, Estimate, fill = phase)) +
    # geom_point(alpha=0.3) +
    stat_summary(fun.y="median", position=position_dodge(width=0.9), geom="bar") +
    # geom_boxplot() +
    # stat_summary(fun.data="mean_se", position=position_dodge(width=0.9), geom="pointrange") +
    facet_grid(~ regr_model) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggsave(file.path(plot_dir, "/6gg_subjwise_regr.png"), gg_subjwise_regr, width = 10, height = 5)
...
all_ACCs$ACC.Missed = ifelse(all_ACCs$item_chosen == "NaN" | is.na(all_ACCs$item_chosen), 1, 0)
...
rainbow_sum$total = with(rainbow_sum, `0` + `1` + `2` + `0and2` + `1and2` + None)
rainbow_fracs = rainbow_sum / rainbow_sum$total  # or rainbow_sum / 12? which does agree with the python version?
...
if (run_on != "simulations") {
  rainbow_sum_l$frac_chosen = rainbow_sum_l$times_chosen / 3  # Every alien is presetend 3 times (1and2, 0and2 have only one opportunity per season)
  rainbow_sum_l$frac_chosen[rainbow_sum_l$TS %in% c("0", "1")] = rainbow_sum_l$frac_chosen[rainbow_sum_l$TS %in% c("0", "1")] / 3  # 3 opportunities per season to pick TS0 and TS1
  rainbow_sum_l$frac_chosen[rainbow_sum_l$TS %in% c("None", "2")] = rainbow_sum_l$frac_chosen[rainbow_sum_l$TS %in% c("None", "2")] / 2  # 2 opportunities per season to pick TS0 and TS1
  ...
  # Add objective TS and alien values
  rainbow_sum_l$item_value_obj = NA
  rainbow_sum_l$item_value_obj[rainbow_sum_l$TS %in% c(0)] = mean(c(6, 5, 4))
  rainbow_sum_l$item_value_obj[rainbow_sum_l$TS %in% c(1)] = mean(c(3, 7, 8))
  rainbow_sum_l$item_value_obj[rainbow_sum_l$TS == 2] = mean(c(3, 3))
  rainbow_sum_l
}
```
...
if (run_on != "simulations") {
  ...
  gg_rainbow_avs = ggplot(rainbow_sum_l, aes(TS)) +
    stat_summary(aes(y = frac_chosen), fun.data="mean_se", geom="pointrange") +#, color = "red"
    # stat_summary(aes(y = times_chosen), color = "blue", fun.data="mean_se", geom="pointrange") +
    labs(y = "Fraction chosen") +# (red); Times chosen (blue)
    facet_grid(model_name ~ .)
...
if (run_on != "simulations") {
  # Model: frac_chosen ~ TS_value_obj + item_value_obj + TS_ACC
  rainbow_sum_l
  rainbow_regr_dat = subset(rainbow_sum_l, TS %in% c("0", "1", "2"))
  rainbow_regr_dat$TS = factor(rainbow_regr_dat$TS)  #factor(rainbow_regr_dat$TS, labels = c("0", "1", "2"), levels = c("0all", "1all", "2"))
  # rainbow_regr_dat$TS[rainbow_regr_dat$TS %in% c("0and2")]
  contrasts(rainbow_regr_dat$TS) = -contr.poly(3)
  
  # One joint model for all agents
  if (run_regression) {
    for (model_namei in unique(rainbow_regr_dat$model_name)) {
        
      rainbow_dat = subset(rainbow_regr_dat, model_name == model_namei)
  
      for (predictor in value_obj_predictors) {
        pred_dat = rainbow_dat[,predictor]
        rainbow_dat[,paste0(predictor, "_zscored")] = (pred_dat - mean(pred_dat, na.rm = T)) / sd(pred_dat, na.rm = T)
      }
    
      rainbow_formula = paste("frac_chosen ~", paste(paste0(value_obj_predictors, "_zscored"), collapse = " + "), "+ (1 | subjID)")  # paste(paste0(value_obj_predictors, "_zscored"), collapse = " + ")
      
      rainbow_lmer = lmer(as.formula(rainbow_formula),
                            data = rainbow_dat,
                            na.action = na.omit,
                            control = lmerControl(optCtrl = list(maxfun = 1e6)))
      lmer_coefs = summary(rainbow_lmer)$coef
      lmer_coefs = cbind(model_name = model_namei, phase = "5RainbowSeason", regr_model = "ACC_value", predictor = rownames(lmer_coefs), as.data.frame(lmer_coefs, row.names = F))
      all_regressions = rbind(all_regressions, lmer_coefs)
    }
  }
  
  for (subj in unique(rainbow_regr_dat$subjID)) {
    
    dat = subset(rainbow_regr_dat, subjID == subj)
...
```
```{r Chosen TS in cloudy - same as rainbow}
# Add all_ACCs columns to rainbow_data
cloudy_all_dat = subset(all_files, phase == "2CloudySeason")
first_trials = ddply(cloudy_all_dat, .(repetition), summarize, first_alien_trial=min(alien_trial))$first_alien_trial
cloudy_dat = subset(cloudy_all_dat, alien_trial %in% first_trials, select = c("subjID", "sad_alien", "item_chosen", "trial_index", "repetition", "alien_trial"))

cloudy_aliens = merge(cloudy_dat, all.x = T, all_ACCs)
cloudy_aliens = cloudy_aliens[order(cloudy_aliens$subjID, cloudy_aliens$trial_index),]
...
cloudy_sum = ddply(cloudy_aliens, .(subjID), summarize,
                    ...
                    Missed = sum(ACC.Missed, na.rm = T),
                    None = sum(ACC.None, na.rm = T))
cloudy_sum$total = with(cloudy_sum, `0` + `1` + `2` + `0and2` + `1and2` + None)
cloudy_fracs = cloudy_sum / cloudy_sum$total  # or cloudy_sum / 12? which does agree with the python version?
cloudy_sum$total = NULL
...
# Add frac_chosen column
if (run_on != "simulations") {
  cloudy_sum_l$frac_chosen = cloudy_sum_l$times_chosen / 3  # Every alien is presetend 3 times (1and2, 0and2 have only one opportunity per season)
  cloudy_sum_l$frac_chosen[cloudy_sum_l$TS %in% c("0", "1")] = cloudy_sum_l$frac_chosen[cloudy_sum_l$TS %in% c("0", "1")] / 3  # 3 opportunities per season to pick TS0 and TS1
  cloudy_sum_l$frac_chosen[cloudy_sum_l$TS %in% c("None", "2")] = cloudy_sum_l$frac_chosen[cloudy_sum_l$TS %in% c("None", "2")] / 2  # 2 opportunities per season to pick TS0 and TS1
  cloudy_sum_l$frac_chosen[cloudy_sum_l$TS == "Missed"] = NA
  
  # Add values (subj and obj) and task performance column
  all_values_perf_sub = subset(all_values_perf, assess == "season" & phase == "Refresher3", select = c("subjID", "name", "id", "value_subj", "value_obj", "av_ACC"))
  cloudy_sum_l = merge(cloudy_sum_l, all.x = T,
                        all_values_perf_sub,
                        by.x = c("subjID", "TS"),
                        by.y = c("subjID", "id"))
  colnames(cloudy_sum_l)[colnames(cloudy_sum_l) %in% c("value_subj", "value_obj")] = c("TS_value_subj", "TS_value_obj")
  if (run_on == "humans") {
    cloudy_sum_l$model_name = "humans"
  } else {
    model_names = subset(parameter_dat, select = c("subjID", "model_name"))
    cloudy_sum_l = merge(cloudy_sum_l, model_names, by = "subjID", all.x = T)
  }
  # Add objective TS and alien values
  cloudy_sum_l$item_value_obj = NA
  cloudy_sum_l$item_value_obj[cloudy_sum_l$TS %in% c(0)] = mean(c(6, 5, 4))
  cloudy_sum_l$item_value_obj[cloudy_sum_l$TS %in% c(1)] = mean(c(3, 7, 8))
  cloudy_sum_l$item_value_obj[cloudy_sum_l$TS == 2] = mean(c(3, 3))
  cloudy_sum_l
}

ggplot(cloudy_sum_l, aes(TS, times_chosen, color=subjID)) +
  stat_summary(fun.data = mean_cl_normal, geom = "bar") +
  stat_summary(fun.data = mean_cl_normal, geom = "pointrange") +
  geom_point(position='jitter', alpha=0.5)
```
```{r Chosen TS in initial learning - same as rainbow}
# Add all_ACCs columns to rainbow_data
initiallearn_all_dat = subset(all_files, phase == "1InitialLearning")
first_trials = ddply(initiallearn_all_dat, .(repetition), summarize, first_alien_trial=min(alien_trial))$first_alien_trial
initiallearn_dat = subset(initiallearn_all_dat, alien_trial %in% first_trials, select = c("subjID", "sad_alien", "item_chosen", "trial_index", "repetition", "alien_trial"))

initiallearn_aliens = merge(initiallearn_dat, all.x = T, all_ACCs)
initiallearn_aliens = initiallearn_aliens[order(initiallearn_aliens$subjID, initiallearn_aliens$trial_index),]

# Create rainbow_sum (count how many action would be correct in each TS)
initiallearn_sum = ddply(initiallearn_aliens, .(subjID), summarize,
                    `0` = sum(ACC.only0, na.rm = T),
                    `1` = sum(ACC.only1, na.rm = T),
                    `2` = sum(ACC.only2, na.rm = T),
                    `0and2` = sum(ACC.02, na.rm = T),
                    ...
                    Missed = sum(ACC.Missed, na.rm = T),
                    None = sum(ACC.None, na.rm = T))
initiallearn_sum$total = with(initiallearn_sum, `0` + `1` + `2` + `0and2` + `1and2` + None)
initiallearn_fracs = initiallearn_sum / initiallearn_sum$total  # or initiallearn_sum / 12? which does agree with the python version?
initiallearn_sum$total = NULL

# Create rainbow_sum_l (put all TS into a single column)
initiallearn_sum_l = melt(initiallearn_sum, id.vars = c("subjID"), variable.name = "TS", value.name = "times_chosen")

# Add frac_chosen column
if (run_on != "simulations") {
  initiallearn_sum_l$frac_chosen = initiallearn_sum_l$times_chosen / 3  # Every alien is presetend 3 times (1and2, 0and2 have only one opportunity per season)
  initiallearn_sum_l$frac_chosen[initiallearn_sum_l$TS %in% c("0", "1")] = initiallearn_sum_l$frac_chosen[initiallearn_sum_l$TS %in% c("0", "1")] / 3  # 3 opportunities per season to pick TS0 and TS1
  initiallearn_sum_l$frac_chosen[initiallearn_sum_l$TS %in% c("None", "2")] = initiallearn_sum_l$frac_chosen[initiallearn_sum_l$TS %in% c("None", "2")] / 2  # 2 opportunities per season to pick TS0 and TS1
  initiallearn_sum_l$frac_chosen[initiallearn_sum_l$TS == "Missed"] = NA
  
  # Add values (subj and obj) and task performance column
  all_values_perf_sub = subset(all_values_perf, assess == "season" & phase == "Refresher3", select = c("subjID", "name", "id", "value_subj", "value_obj", "av_ACC"))
  initiallearn_sum_l = merge(initiallearn_sum_l, all.x = T,
                        all_values_perf_sub,
                        by.x = c("subjID", "TS"),
                        by.y = c("subjID", "id"))
  colnames(initiallearn_sum_l)[colnames(initiallearn_sum_l) %in% c("value_subj", "value_obj")] = c("TS_value_subj", "TS_value_obj")
  if (run_on == "humans") {
    initiallearn_sum_l$model_name = "humans"
  } else {
    model_names = subset(parameter_dat, select = c("subjID", "model_name"))
    initiallearn_sum_l = merge(initiallearn_sum_l, model_names, by = "subjID", all.x = T)
  }
  # Add objective TS and alien values
  initiallearn_sum_l$item_value_obj = NA
  initiallearn_sum_l$item_value_obj[initiallearn_sum_l$TS %in% c(0)] = mean(c(6, 5, 4))
  initiallearn_sum_l$item_value_obj[initiallearn_sum_l$TS %in% c(1)] = mean(c(3, 7, 8))
  initiallearn_sum_l$item_value_obj[initiallearn_sum_l$TS == 2] = mean(c(3, 3))
  initiallearn_sum_l
}

ggplot(initiallearn_sum_l, aes(TS, times_chosen, color=subjID)) +
  stat_summary(fun.data = mean_cl_normal, geom = "bar") +
  stat_summary(fun.data = mean_cl_normal, geom = "pointrange") +
  geom_point(position='jitter', alpha=0.5)
```
...
if (run_on == "humans" & substr(data_dir, 55, 58) != "1.0") {
  ...
  make_mixed_dataframes = function(dat) {
    mixed_block_l = melt(dat, measure.vars = c("ts", "alien", "both"), variable.name = "level", value.name = "switch")
    mixed_block_l$level = factor(mixed_block_l$level, levels = c("alien", "ts", "both"))
    mixed_block_l_sum = ddply(subset(mixed_block_l, level == "ts" & !is.na(switch)), .(subjID, switch), summarize,
          n_trials = length(switch),
          ts_RT = median(rt, na.rm = T),
    ...
    mixed_block_l_sum_wide$RT_diff = with(mixed_block_l_sum_wide, ts_RT.Switch - ts_RT.Stay)
    mixed_block_l_sum_wide$ACC_diff = with(mixed_block_l_sum_wide, ts_ACC.Switch - ts_ACC.Stay)
    return(list(mixed_block_l, mixed_block_l_sum, mixed_block_l_sum_wide))
  }
  
  # Get data
  all_three = make_mixed_dataframes(mixed_block)
  mixed_block_l = all_three[[1]]
  mixed_block_l_sum = all_three[[2]]
  mixed_block_l_sum_wide = all_three[[3]]
  
  # Split data into two blocks
  mixed_trial_idx = unique(mixed_block$trial_index)
  half_trial_idx = round(mean(mixed_trial_idx))
  
  first_all_three = make_mixed_dataframes(subset(mixed_block, trial_index < half_trial_idx))
  first_mixed_block_l = first_all_three[[1]]
  first_mixed_block_l_sum = first_all_three[[2]]
  first_mixed_block_l_sum_wide = first_all_three[[3]]
  
  second_all_three = make_mixed_dataframes(subset(mixed_block, trial_index >= half_trial_idx))
  second_mixed_block_l = second_all_three[[1]]
  second_mixed_block_l_sum = second_all_three[[2]]
  second_mixed_block_l_sum_wide = second_all_three[[3]]
...
if (run_on == "humans" & substr(data_dir, 55, 58) != "1.0") {
  ...
  half_block = (min(mixed_block_l$trial_index) + max(mixed_block_l$trial_index)) / 2
  mixed_block_l$half = "First Half"
  mixed_block_l$half[mixed_block_l$trial_index > half_block] = "Second Half"
  ...
  gg_mixed_rts = ggplot(subset(mixed_block_l, !is.na(switch) & ACC == 1 & level != 'both'), aes(level, rt, color = level)) +
    stat_summary(fun.data = mean_se, geom = "pointrange", position = position_dodge(width = 0.9)) +
    labs(x = "", y = "RT (msec)", fill = "") +
    theme(legend.position = "none") +
    facet_grid(half ~ switch)
...
dat = ddply(mixed_block, .(subjID), summarize, ACC=mean(ACC))
mean(dat$ACC)
sd(dat$ACC)
```

# Paper stats and tests

```{r Average performance}
subj_dat = ddply(subset(all_files, phase=="1InitialLearning"), .(subjID), summarize, ACC=mean(ACC, na.rm=T))
length(unique(subj_dat$subjID))
mean(subj_dat$ACC)
sd(subj_dat$ACC)
```

```{r assymetric switch costs}
if (substr(data_dir, 55, 58) != "1.0") {
  ...
  # Tests
  hist(mixed_block_l_sum_wide$RT_diff)  # with(mixed_block_l_sum_wide, ts_RT.Switch - ts_RT.Stay)
  print(t.test(mixed_block_l_sum_wide$RT_diff))
  
  library("effsize")
  print(cohen.d(mixed_block_l_sum_wide$ts_RT.Switch, mixed_block_l_sum_wide$ts_RT.Stay))
  
  hist(mixed_block_l_sum_wide$ACC_diff)
  print(t.test(mixed_block_l_sum_wide$ACC_diff))
  
  # Based on half-blocks (first influence by previous blocked expectations, second one clean)
  hist(first_mixed_block_l_sum_wide$RT_diff)
  print(t.test(first_mixed_block_l_sum_wide$RT_diff))
  
  hist(second_mixed_block_l_sum_wide$RT_diff)
  print(t.test(second_mixed_block_l_sum_wide$RT_diff))
  
  t.test(first_mixed_block_l_sum_wide$RT_diff - second_mixed_block_l_sum_wide$RT_diff)
}
```

```{r t-tests for intrusion errors and accuracy}
sum_dat = ddply(subset(persev_dat2, alien_trial_sum==1 & phase=="1InitialLearning"), .(subjID, ACC_TS), summarize, ACC=mean(ACC, na.rm=T))
ddply(sum_dat, .(ACC_TS), summarize, ACC=mean(ACC,na.rm=T))
wide_dat = reshape(sum_dat, direction='wide', timevar='ACC_TS', idvar='subjID')
wide_dat$ACC_not_TS = with(wide_dat, 0.5 * (ACC.ACC_prev_TS + ACC.ACC_prev_notTS))
wide_dat$ACC_minus_Intr = with(wide_dat, ACC.ACC - ACC_not_TS)

ggplot(wide_dat, aes(ACC.ACC_prev_TS)) + geom_histogram()
ggplot(wide_dat, aes(ACC_minus_Intr)) + geom_histogram()
t.test(wide_dat$ACC.ACC_prev_TS - 1/3)
t.test(wide_dat$ACC.ACC_prev_notTS - 1/3)
t.test(wide_dat$ACC_minus_Intr)
cohen.d(wide_dat$ACC.ACC, wide_dat$ACC_not_TS)
```
```{r regressions performance ~ action values and task-set values and cloudy}
# Effect of action values and task-set values on performance
colnames(feed_aliens)
act_ts_val_mode = glmer(correct ~ item_value_obj * TS_value_obj + (1 | subjID) + (1 | alien_trial) + (1 | repetition),
                        subset(feed_aliens, phase=="1InitialLearning"), binomial)
summary(act_ts_val_mode)

# Reactivation of task-sets
feed_alien_trial1$TS = as.numeric(as.character(feed_alien_trial1$TS))
react_4trials_mod = glmer(ACC ~ n_seen_aliens + TS + (1 | subjID) + (1 | block),
                          subset(feed_alien_trial1, phase=="2CloudySeason"),
                          binomial)
summary(react_4trials_mod)

sum_react_dat = ddply(subset(feed_alien_trial1, phase=="2CloudySeason"), .(subjID, phase, n_seen_aliens), ACC=mean(ACC, na.rm=T), summarize)
ggplot(sum_react_dat, aes(ACC, n_seen_aliens)) + geom_point(position="jitter", alpha=0.1)
cor.test(sum_react_dat$ACC, sum_react_dat$n_seen_aliens)

reactivation_mod = glmer(ACC ~ ACC_1back_self + ACC_1back_others + (1 | subjID) + (1 | alien_trial_sum) + (1 | repetition),
            subset(ACC_1back_dat, phase=="1InitialLearning"),
            binomial)
summary(reactivation_mod)

reactivation_mod_IL = update(reactivation_mod, data=subset(ACC_1back_dat, phase=="1InitialLearning"))
summary(reactivation_mod_IL)
```
```{r repeated-measures t-test for TS value effect in IL}
dat = ddply(subset(feed_aliens, item_value_obj %in% c(3, 7) & phase=="1InitialLearning"), .(subjID, TS_value_obj, item_value_obj), summarize, ACC=mean(ACC, na.rm=T))
t.test(ACC ~ TS_value_obj, dat)
wide_dat = reshape(dat, direction = "wide", timevar = "TS_value_obj", idvar = c("subjID", "item_value_obj"))
wide_dat$ACC2minus1 = with(wide_dat, ACC.3.75 - ACC.5)
summary(lm(ACC ~ TS_value_obj + item_value_obj, dat))
ggplot(wide_dat, aes(ACC2minus1, fill=item_value_obj, group=item_value_obj)) + geom_histogram(position=position_dodge())
sum_dat = ddply(wide_dat, .(subjID), summarize, ACC1=mean(ACC.3.75, na.rm=T), ACC2=mean(ACC.5, na.rm=T), ACC2minus1=mean(ACC2minus1, na.rm=T))
t.test(sum_dat$ACC2minus1)
ddply(sum_dat, .(), summarize, ACC1=mean(ACC1), ACC2=mean(ACC2), diff=ACC2-ACC1)
```
```{r Competiton phase}
if (substr(data_dir, 55, 58) != "1.0") {
  subj_dat = ddply(pick_aliens, .(subjID, assess), summarize, ACC=mean(selected_better_obj, na.rm=T))
  subj_dat_wide = reshape(subset(subj_dat, assess%in%c('season', 'alien-same-season')), direction='wide', timevar='assess', idvar='subjID')
  subj_dat_wide$context_minus_stimulus = with(subj_dat_wide, ACC.season - `ACC.alien-same-season`)
  sum_dat = ddply(subj_dat, .(assess), summarize, ACC=mean(ACC))
  
  print(mean(subj_dat_wide$`ACC.season`))
  print(t.test(subj_dat_wide$`ACC.season`-0.5))
  print(cohen.d(subj_dat_wide$`ACC.season`, 1-subj_dat_wide$`ACC.season`))
  
  print(mean(subj_dat_wide$`ACC.alien-same-season`))
  print(t.test(subj_dat_wide$`ACC.alien-same-season`-0.5))
  print(cohen.d(subj_dat_wide$`ACC.alien-same-season`, 1-subj_dat_wide$`ACC.alien-same-season`))
  
  print(t.test(subj_dat_wide$`ACC.alien-same-season` - subj_dat_wide$`ACC.season`))
  
  unique(all_files$phase)
  dat = ddply(subset(all_files, phase=='3PickAliens' & assess%in%c('season', 'alien-same-season')), .(subjID, assess), summarize, RT=median(rt, na.rm=T), missed=sum(is.na(item_chosen_name)))
  wide_dat = reshape(dat, direction='wide', timevar='assess', idvar='subjID')
  wide_dat$RT_context_minus_season = with(wide_dat, `RT.alien-same-season` - RT.season)
  wide_dat$missed_context_minus_season = with(wide_dat, `missed.alien-same-season` - missed.season)
  ggplot(dat, aes(assess, RT)) + geom_point()
  print(t.test(wide_dat$RT_context_minus_season))
  ggplot(dat, aes(assess, missed)) + geom_point(position='jitter')
  print(t.test(wide_dat$missed_context_minus_season))
  
  summary(glmer(selected_better_obj ~ assess + (1 | subjID), family=binomial, data=subset(pick_aliens, assess%in%c('season', 'alien-same-season')), na.action=na.omit))
  summary(lmer(RT ~ assess + (1 | subjID), data=dat, na.action=na.omit))
  
  t.test(log(wide_dat$`RT.alien-same-season`) - log(wide_dat$RT.season))
}
```
```{r Rainbow phase}
...
rainbow_sum$`2minus0` = with(rainbow_sum, `2` - `0`)
rainbow_sum$`2minus1` = with(rainbow_sum, `2` - `1`)
rainbow_sum$`1minus0` = with(rainbow_sum, `1` - `0`)
rainbow_sum$`21minus20` = with(rainbow_sum, `0and2` - `1and2`)
t.test(rainbow_sum$`2minus0`)
cohen.d(rainbow_sum$`2`, rainbow_sum$`0`)
t.test(rainbow_sum$`2minus1`)
cohen.d(rainbow_sum$`0`, rainbow_sum$`1`)
t.test(rainbow_sum$`1minus0`)
t.test(rainbow_sum$`21minus20`)
cohen.d(rainbow_sum$`1and2`, rainbow_sum$`0and2`)

mean(rainbow_sum$`0`)
mean(rainbow_sum$`1`)
mean(rainbow_sum$`2`)

rainbow_sum$`2TS` = with(rainbow_sum, `0and2` + `1and2`)
rainbow_sum$`1TS` = with(rainbow_sum, `0` + `1` + `2`)
rainbow_sum$`2TSminus1TS` = with(rainbow_sum, `2TS` - `1TS`)
t.test(rainbow_sum$`2TS` - 2 / 12 * Valid_choices)
t.test(rainbow_sum$`2TSminus1TS` - (2 / 12 * Valid_choices - 8 / 12 * Valid_choices))

mean(rainbow_sum$`2TS`) / 12
mean(rainbow_sum$None) / 12
...
Double_TS_choices = rainbow_sum$`0and2` + rainbow_sum$`1and2`
mean(Double_TS_choices)
mean(Double_TS_choices / Valid_choices)
t.test(Double_TS_choices - 2 / 12 * Valid_choices)
Single_TS_choices = rainbow_sum$`0` + rainbow_sum$`1` + rainbow_sum$`2`

t.test(Double_TS_choices / 2 - Single_TS_choices / 8)

```
```{r Diagnostic Trials}
# Mark "non-diagnostic" trials (same action is correct for an alien in TWO DIFFERENT task-sets)
feed_aliens$correct = as.logical(feed_aliens$correct)
feed_aliens$diagnostic_trial = TRUE
feed_aliens$diagnostic_trial[feed_aliens$sad_alien == 0 & feed_aliens$TS == 2] = FALSE  # alien: 0, item: 2, reward: 7
feed_aliens$diagnostic_trial[feed_aliens$sad_alien == 0 & feed_aliens$TS == 1] = FALSE  # alien: 0, item: 2, reward: 2
feed_aliens$diagnostic_trial[feed_aliens$sad_alien == 3 & feed_aliens$TS == 0] = FALSE  # alien: 3, item: 0, reward: 10
feed_aliens$diagnostic_trial[feed_aliens$sad_alien == 3 & feed_aliens$TS == 0] = FALSE  # alien: 3, item: 0, reward: 2

# Find trials that follow diagnostic and non-diagnostic trials
feed_aliens$after_diagnostic = c(NA, feed_aliens$diagnostic_trial[1:length(feed_aliens$diagnostic_trial)-1])

ACC_1back = c(NA, as.logical(feed_aliens$correct[1:length(feed_aliens$correct)-1]))
ACC_1back[feed_aliens$trial_index == 44] = NA
feed_aliens$ACC_1back = ACC_1back

# Compare performance
diag_dat = ddply(
  subset(feed_aliens, ACC_1back == TRUE & after_diagnostic == TRUE & phase == "2CloudySeason"),
  .(subjID), summarize,
  ACC=mean(correct, na.rm=TRUE))
nondiag_dat = ddply(
  subset(feed_aliens, ACC_1back == TRUE & after_diagnostic == FALSE & phase == "2CloudySeason"),
  .(subjID), summarize,
  ACC=mean(correct, na.rm=TRUE))
diag_data = merge(diag_dat, nondiag_dat, by="subjID", suff=c("_diag", "_nondiag"))

# Performance after diag and non-diag
summary(diag_data)
t.test(diag_data$ACC_diag - diag_data$ACC_nondiag)
```
...
ACCfirst8_correct_dat = ddply(subset(feed_aliens, alien_trial_sum == (n_trials / n_aliens + 1)), .(subjID, phase), summarize,
...
if (ACCfirst8_correct_dat$model_name[1] != "human") {
  gg_alpha_ACCfirst8 =
    ...
    facet_grid(model_name ~ .)
  gg_alphahigh_ACCfirst8 = gg_alpha_ACCfirst8 + aes(x = alpha_high)
  gg_beta_ACCfirst8 = gg_alpha_ACCfirst8 + aes(x = beta)
  gg_betahigh_ACCfirst8 = gg_alpha_ACCfirst8 + aes(x = beta_high)
  ...
  if (gg_save) {
    ...
    ggsave(file.path(plot_dir, "/6gg_alphahigh_ACCfirst8.png"), gg_alphahigh_ACCfirst8, width = 4, height = 3 * length(unique(all_files$model_name)))
    ggsave(file.path(plot_dir, "/6gg_beta_ACCfirst8.png"), gg_beta_ACCfirst8, width = 4, height = 3 * length(unique(all_files$model_name)))
    ggsave(file.path(plot_dir, "/6gg_betahigh_ACCfirst8.png"), gg_betahigh_ACCfirst8, width = 4, height = 3 * length(unique(all_files$model_name)))
...
gg_ACCfirst8 =
  ...
  stat_summary(fun.data = mean_se, geom = "pointrange") +
  labs(x = "", y = "Switch speed (ACC in first 2 alien trials)", fill = "") +
  theme(legend.position = "none") +
  facet_grid(model_name ~ .)
ggsave(file.path(plot_dir, "/6gg_ACCfirst8.png"), gg_ACCfirst8, width = 8, height = 3 * length(unique(all_files$model_name)))
```
...
gg_alphahigh_pickaliens = gg_alpha_pickaliens + aes(x = alpha_high)
gg_beta_pickaliens = gg_alpha_pickaliens + aes(x = beta)
gg_betahigh_pickaliens = gg_alpha_pickaliens + aes(x = beta_high)
gg_forget_pickaliens = gg_alpha_pickaliens + aes(x = forget)
gg_biasedTS_pickaliens = gg_alpha_pickaliens + aes(x = create_TS_biased)
gg_alpha_min_alphahigh_pickaliens = gg_alpha_pickaliens + aes(x = alpha - alpha_high)
gg_beta_min_betahigh_pickaliens = gg_alpha_pickaliens + aes(x = beta - beta_high)

if (gg_save) {
  ggsave(file.path(plot_dir, "/6gg_alphahigh_pickaliens.png"), gg_alphahigh_pickaliens, width = 10, height = 2 * length(unique(all_files$model_name)))
  ...
  ggsave(file.path(plot_dir, "/6gg_betahigh_pickaliens.png"), gg_betahigh_pickaliens, width = 10, height = 2 * length(unique(all_files$model_name)))
  ...
  ggsave(file.path(plot_dir, "/6gg_alpha_min_alphahigh_pickaliens.png"), gg_alpha_min_alphahigh_pickaliens, width = 10, height = 2 * length(unique(all_files$model_name)))
  ggsave(file.path(plot_dir, "/6gg_beta_min_betahigh_pickaliens.png"), gg_beta_min_betahigh_pickaliens, width = 10, height = 2 * length(unique(all_files$model_name)))
}
}
```
...
gg_values =
  ggplot(subset(subjwise_regr_coefs, predictor %in% interesting_predictors), aes(phase, Estimate, color = phase, group = phase, alpha = `Pr(>|z|)` < 0.05)) +
  # geom_violin() +
...
if (length(models) > 1) {
  for (model in models) {
    gg_alpha_values =
      ggplot(subset(subjwise_regr_coefs, regr_model == model & predictor != "(Intercept)"), aes(alpha, Estimate, color = phase, group = phase)) +
      geom_point() +
      geom_smooth() +
      geom_hline(yintercept = 0, linetype = "dotted") +
      ylim(-4, 4) +
      facet_grid(model_name ~ predictor)
    gg_alphahigh_values = gg_alpha_values + aes(x = alpha_high)
    gg_beta_values = gg_alpha_values + aes(x = beta)
    gg_betahigh_values = gg_alpha_values + aes(x = beta_high)
    gg_TSbiased_values = gg_alpha_values + aes(x = create_TS_biased)
    gg_forget_values = gg_alpha_values + aes(x = forget)
    gg_alpha_min_alphahigh = gg_alpha_values + aes(x = (alpha - alpha_high))
    gg_beta_min_betahigh = gg_alpha_values + aes(x = (beta - beta_high))
    ...
    if (gg_save) {
      ...
      ggsave(file.path(plot_dir, paste0("/6gg_alphahigh_values_", model, ".png")), gg_alphahigh_values, width = 10, height = 3 * length(unique(all_files$model_name)))
      ggsave(file.path(plot_dir, paste0("/6gg_alpha_min_alphahigh_values_", model, ".png")), gg_alpha_min_alphahigh, width = 10, height = 3 * length(unique(all_files$model_name)))
      ggsave(file.path(plot_dir, paste0("/6gg_beta_min_betahigh_values_", model, ".png")), gg_beta_min_betahigh, width = 10, height = 3 * length(unique(all_files$model_name)))
      ...
      ggsave(file.path(plot_dir, paste0("/6gg_betahigh_values_", model, ".png")), gg_betahigh_values, width = 10, height = 3 * length(unique(all_files$model_name)))
...
```
```{r Grant plots}
gg_grant_item_values = gg_learning_curves_item_value_obj
gg_grant_item_values$data = subset(gg_grant_item_values$data, phase == "1InitialLearning")

gg_grant_TS_values = gg_learning_curves_TS_value_obj
gg_grant_TS_values$data = subset(gg_grant_TS_values$data, phase == "1InitialLearning")

gg_grant_competition = gg_pick_aliens_value_subj + labs(y = "Preference") + theme(legend.position = "none") + coord_cartesian(ylim = c(-1.1, 2.3))
gg_grant_competition$data$assess = factor(gg_grant_competition$data$assess, levels = c("alien-same-season", "season", "alien", "item"), labels = c("C-S", "C", "S", "R"))

gg_grant_rainbow = gg_rainbow_avs + labs(x = "")
gg_grant_rainbow$data = subset(gg_grant_rainbow$data, TS %in% c("0", "1", "2", "None"))
gg_grant_rainbow$data$TS = factor(gg_grant_rainbow$data$TS, levels = c("0", "1", "2", "None"), labels = c("High", "Med", "Low", "None"))

width_and_height = 2

ggsave(file.path(plot_dir, "/11gg_grant_item_values.png"), gg_grant_item_values, width = width_and_height, height = width_and_height)
ggsave(file.path(plot_dir, "/11gg_grant_TS_values.png"), gg_grant_TS_values, width = width_and_height, height = width_and_height)
ggsave(file.path(plot_dir, "/11gg_grant_competition.png"), gg_grant_competition, width = width_and_height, height = width_and_height)
ggsave(file.path(plot_dir, "/11gg_grant_rainbow.png"), gg_grant_rainbow, width = width_and_height, height = width_and_height)
```
```{r Start button}
```