...
regr_dat = subset(feed_aliens, select = c("subjID", "phase", "sad_alien", "TS", "reward", "av_reward", "alien_trial_sum", "alien_trial", "ACC"))
...
for (alien in unique(regr_dat$sad_alien)) {
  alien_regr_dat = ddply(subset(regr_dat, sad_alien != alien),
                    .(subjID, phase, TS, alien_trial), summarize,
...
regr_dat = merge(others_regr_dat, regr_dat,  # sanity check: cloudy regr_dat should have 12000 rows: 50 subj * 20 alien_trials * 3 TS * 4 sad_aliens
                 by = c("subjID", "phase", "TS", "alien_trial", "sad_alien"))
...
for (phas in c("1InitialLearning", "2CloudySeason")) {
  ...
  dat$sad_alien = factor(as.character(dat$sad_alien))
  
  learning_curves_value = ggplot(dat, aes(alien_trial, 100 * ACC, color = av_reward, group = av_reward)) +
    stat_summary(fun.data = mean_se, geom = "pointrange") +
    geom_hline(yintercept = 100/3, linetype = "dotted") +
  ...
  learning_value_RTs = learning_curves_TS_aliens + aes(trial_index, rt)
