...
for (subj in unique(persev_dat$subjID)) {
  for (phas in feed_aliens_phases) {
    ...
    intrusion_formula = paste("ACC ~", paste(value_obj_predictors, collapse = " + "), "+", paste(predictors_no_interest, collapse = " + "))
...
if (run_regression) {
  for (model_namei in unique(ACC_1back_dat$model_name)) {
    for (phas in feed_aliens_phases) {
      ...
      value_formula = paste("ACC ~", paste(value_obj_predictors, collapse = " + "), "+", paste(predictors_no_interest, collapse = " + "),
                            "+ (", paste(value_obj_predictors, collapse = " + "), "+", paste(predictors_no_interest, collapse = " + "))
      nback_formula = paste("ACC ~", paste(nback_predictors, collapse = " + "), "+", paste(predictors_no_interest, collapse = " + "),
                            "+ (", paste(nback_predictors, collapse = " + "), "+", paste(predictors_no_interest, collapse = " + "))
      ...
      value_coefs = cbind(model_name = model_namei, phase = phas, regr_model = "intrusions", predictor = rownames(value_coefs), as.data.frame(value_coefs, row.names = F))

      nback_glmer = update(value_glmer, formula = as.formula(nback_formula))
      nback_coefs = summary(nback_glmer)$coefs
      nback_coefs = cbind(model_name = model_namei, phase = phas, regr_model = "intrusions", predictor = rownames(nback_coefs), as.data.frame(nback_coefs, row.names = F))
      
      rew1back_glmer = update(value_glmer, formula = as.formula(rew1back_formula))
      rew1back_coefs = summary(rew1back_glmer)$coefs
      rew1back_coefs = cbind(model_name = model_namei, phase = phas, regr_model = "intrusions", predictor = rownames(rew1back_coefs), as.data.frame(rew1back_coefs, row.names = F))
...
gg_alpha_6_of_8 =
  ggplot(six_of_8_correct_dat, aes(alpha, trial_reached, color = phase)) +
  ...
  facet_grid(model_name ~ .)
gg_beta_6_of_8 = gg_alpha_6_of_8 + aes(x = beta)
gg_forget_6_of_8 = gg_alpha_6_of_8 + aes(x = forget)
gg_TSbias_6_of_8 = gg_alpha_6_of_8 + aes(x = create_TS_biased)
...
beta_limits = quantile(subjwise_regr_coefs$Estimate, c(.15, .85), na.rm = T)
subjwise_regr_coefs = subset(subjwise_regr_coefs, Estimate > beta_limits[1] & Estimate < beta_limits[2])

gg_alpha_values =
  ggplot(subjwise_regr_coefs, aes(alpha, Estimate, color = phase, group = phase)) +
  geom_point() +
  geom_smooth() +
  geom_hline(yintercept = 0, linetype = "dotted") +
