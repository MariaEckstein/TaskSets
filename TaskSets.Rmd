...
if (run_on == "humans") {
  ...
  data_dir = "C:/Users/maria/MEGAsync/SLCN/PSsimulations/"   # "C:/Users/maria/MEGAsync/Berkeley/TaskSets/AlienGenRec/MaxInsteadOfSoftmax/"
  patt = "aliens_hier.*csv"  # "alien_..csv" -> subjID1-9, "alien_...csv" -> subjID10-99, "alien_....csv" -> subjID100-999
...
subset(dat, subjID < 100)  # make sure version1.0 data is there
```
...
gg_learning_curves_TS_value_obj =
  ggplot(subset(feed_aliens, phase != "0Training"), aes(alien_trial_sum, 100 * ACC, color = TS_value_obj, group = TS_value_obj)) +
  stat_summary(fun.data = mean_se, geom = "pointrange") +
...
if (gg_save) {
  ...
  ggsave(file.path(plot_dir, "/2gg_learning_curves_TS_value_obj.png"), gg_learning_curves_TS_value_obj, width = 2.7, height = 3 * length(unique(all_files$model_name)))
  ...
  ggsave(file.path(plot_dir, "/2gg_learning_curves_item_value_obj.png"), gg_learning_curves_item_value_obj, width = 2.7, height = 3 * length(unique(all_files$model_name)))
...
reactivation_mod = glmer(ACC ~ ACC_1back_self + ACC_1back_others + (1 | subjID) + (1 | alien_trial_sum) + (1 | repetition),
            subset(ACC_1back_dat, phase=="2CloudySeason"),
            binomial)
