...
gg_save = F
run_regression = F
max_n_subj = 31  # versions1.0and3.1 -> 82; version3.1 -> 31
...
if (run_on == "humans") {
  data_dir = "C:/Users/maria/MEGAsync/Berkeley/TaskSets/Data/version3.1"  # version3.1  # versions1.0and3.1
  patt = "aliens.*csv"
...
plot_dir = file.path(data_dir, "alien_plots_cloudy")
n_trials
n_blocks
...
filenames[1:max_n_subj]
all_files = data.frame()

for (filename in filenames[1:max_n_subj]) {
...
correct_nrows = assertthat::are_equal(nrow(subj_file) * length(filenames[1:max_n_subj]), nrow(all_files))
if (!correct_nrows) {
  warning(paste("all_files does not have the expected number of rows. Expected: nrow(subj_file) * length(filenames[1:max_n_subj]) =", nrow(subj_file), "*", length(filenames[1:max_n_subj]), "=", nrow(subj_file) * length(filenames[1:max_n_subj]), "- Actual:", nrow(all_files)))
}
...
#   train_dat
#   summary(train_dat)
#   perf_dat = ddply(subset(train_dat, trial_index %in% 30:41),  # 7th repetition of each alien starts at trial 28; training has 41 trials total
#                    .(subjID), summarize,
...
```
```{r Save preprocessed pick_aliens}
print(paste0("Saving preprocessed files to ", data_dir))
for (subj in unique(all_files$subjID)) {
...
if (run_on == "humans") {
  ...
  t.test(mixed_block_l_sum_wide$RT_diff)
  
  hist(mixed_block_l_sum_wide$ACC_diff)
  t.test(mixed_block_l_sum_wide$ACC_diff)
