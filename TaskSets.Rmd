...
ggplot(subset(feed_aliens, subjID == unique(all_files$subjID)[1] & trial_index < 500), aes(trial_index, rolling_average_ACC, color = block %% 3)) +
  geom_point() +
...
gg_TSbias_6_of_8 = gg_alpha_6_of_8 + aes(x = 2 * round(create_TS_biased / 2, 1)) + labs(x = "Biased TS (binned)")

if (gg_save) {
  ggsave(file.path(plot_dir, "/6gg_alpha_six_of_8.png"), gg_alpha_6_of_8, width = 4, height = 3 * length(unique(all_files$model_name)))
  ggsave(file.path(plot_dir, "/6gg_beta_six_of_8.png"), gg_beta_6_of_8, width = 4, height = 3 * length(unique(all_files$model_name)))
  ggsave(file.path(plot_dir, "/6gg_forget_six_of_8.png"), gg_forget_6_of_8, width = 4, height = 3 * length(unique(all_files$model_name)))
  ggsave(file.path(plot_dir, "/6gg_TSbias_6_of_8.png"), gg_TSbias_6_of_8, width = 4, height = 3 * length(unique(all_files$model_name)))
}
```
...
gg_TSbiased_values = gg_alpha_values + aes(x = 2 * round(create_TS_biased / 2, 1)) + labs(x = "Biased TS creation (binned)")

if (gg_save) {
  ggsave(file.path(plot_dir, "/6gg_alpha_values.png"), gg_alpha_values, width = 20, height = 3 * length(unique(all_files$model_name)))
  ggsave(file.path(plot_dir, "/6gg_beta_values.png"), gg_beta_values, width = 20, height = 3 * length(unique(all_files$model_name)))
  ggsave(file.path(plot_dir, "/6gg_TSbiased_values.png"), gg_TSbiased_values, width = 20, height = 3 * length(unique(all_files$model_name)))
}
```
```{r Start button}
```